...
# Largest per-step target offset (rad). Matches the training clip of +-ACTION_SCALE, so by
# default this changes nothing about what the policy's actions do. Joint *speed* is capped by
# the servos' GOAL_SPEED (0.92 rad/s), the same limit velocity_limit_sim now imposes in training.
# Lower it with --max-delta for a cautious first run, knowing that departs from training.
...
def main() -> None:
    ...
    with SO101Bus(args.port, args.baud, allow_motion=args.allow_motion,
        ...
        for step in range(n_steps):
            ...
            # travels at up to velocity_limit_sim. (An earlier version integrated a separate
            # setpoint to get past the servo deadband with 0.02 rad offsets; with the full
            # 0.25 rad offset that is no longer needed, and it made real behaviour differ
            # from training.)
